...
(function (f, n) {
  ...
  b.find_uri_expression = /\b((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?\u00ab\u00bb\u201c\u201d\u2018\u2019]))/gi;
  b.findUri = {
  ...
  var y = { encode: 'encode', decode: 'decode' },
    ...
    F = function (a, c) {
      return function (d) {
        try {
          return b[c](d + '').replace(b.characters[a][c].expression, function (
            d
          ) {
  ...
  b.build = function (a) {
    ...
    a.urn || (!c && !a.hostname) || ((c += '//'), (d = !0));
    c += b.buildAuthority(a) || '';
    'string' === typeof a.path &&
      ('/' !== a.path.charAt(0) && d && (c += '/'), (c += a.path));
  ...
  b.withinString = function (a, c, d) {
    ...
    var l = d.start || b.findUri.start,
      e = d.end || b.findUri.end,
      g = d.trim || b.findUri.trim,
      f = d.parens || b.findUri.parens,
      h = /[a-z0-9-]=["']?$/i;
    for (l.lastIndex = 0; ; ) {
      var k = l.exec(a);
      if (!k) break;
      var p = k.index;
      if (d.ignoreHtml) {
        var n = a.slice(Math.max(p - 3, 0), p);
        if (n && h.test(n)) continue;
      }
      var r = p + a.slice(p).search(e);
      n = a.slice(p, r);
      for (r = -1; ; ) {
        var q = f.exec(n);
        if (!q) break;
        r = Math.max(r, q.index + q[0].length);
      }
      n = -1 < r ? n.slice(0, r) + n.slice(r).replace(g, '') : n.replace(g, '');
      n.length <= k[0].length ||
        ...
        void 0 === k
          ? (l.lastIndex = r)
          : ((k = String(k)),
            (a = a.slice(0, p) + k + a.slice(r)),
            (l.lastIndex = p + k.length)));
    }
    l.lastIndex = 0;
    return a;
  };
